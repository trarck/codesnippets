...
const long int MessageGlobalObject=0xFFFFFFFFFFFFFFFF;

CCMessageManager::CCMessageManager():
m_messages(NULL),
m_globalObject(MessageGlobalObject)
{
...
bool CCMessageManager::registerReceiver(MessageParty receiver,SEL_MessageHandler handle ,MessageType type ,MessageParty sender ,CCObject*  handleObject)
{
...
//关注的对象是receiver
bool CCMessageManager::registerReceiver(MessageParty receiver,SEL_MessageHandler handle,MessageType type ,MessageParty sender)
{
...
void CCMessageManager::removeReceiver(MessageParty receiver,MessageType type ,MessageParty sender,SEL_MessageHandler handle)
{
...
void CCMessageManager::removeReceiver(MessageParty receiver,MessageType type ,MessageParty sender)
{
    CCAssert(receiver!=NULL,"CCMessageManager:removeReceiver:receiver can't be null!");
...
void CCMessageManager::removeReceiver(MessageParty receiver,MessageType type ,SEL_MessageHandler handle)
{
    CCAssert(receiver!=NULL,"CCMessageManager:removeReceiver:receiver can't be null!");
...
void CCMessageManager::removeReceiver(MessageParty receiver,MessageType type)
{
    CCAssert(receiver!=NULL,"CCMessageManager:removeReceiver:receiver can't be null!");
	CCDictionary *msgMap=(CCDictionary*) m_messages->objectForKey(type);
	if(msgMap){
...
void CCMessageManager::removeReceiver(MessageParty receiver)
{
    CCAssert(receiver!=NULL,"CCMessageManager:removeReceiver:receiver can't be null!");
...
void CCMessageManager::removeReceiver(MessageParty receiver,MessageParty sender,SEL_MessageHandler handle)
{
    CCAssert(receiver!=NULL,"CCMessageManager:removeReceiver:receiver can't be null!");
...
void CCMessageManager::removeReceiver(MessageParty receiver,MessageParty sender)
{
    CCAssert(receiver!=NULL,"CCMessageManager:removeReceiver:receiver can't be null!");
...
void CCMessageManager::removeReceiver(MessageParty receiver,SEL_MessageHandler handle)
{
...
void CCMessageManager::removeReceiverMap(MessageParty receiver,CCDictionary* map,SEL_MessageHandler handle){
	if (map) {
...
void CCMessageManager::removeReceiverMap(MessageParty receiver,CCDictionary* map){
	if (map) {
        CCArray *receiverList=(CCArray*)map->objectForKey(receiver->m_uID);
        if(receiverList){
            removeReceiverList(receiverList);
...
{
	MessageParty receiver=message->getReceiver();
	if (receiver) {
...
void CCMessageManager::execRegisterWithSenderMap(CCDictionary* senderMap,CCMessage* message,MessageParty  receiver)
{
...
void CCMessageManager::dispatchMessageWithType(MessageType type ,MessageParty sender ,MessageParty receiver,CCObject* data)
{
...
void CCMessageManager::dispatchMessageWithType(MessageType type ,MessageParty sender ,MessageParty receiver,CCDictionary* data)
{
	CCMessage* message=new CCMessage();
	message->initWithType(type ,sender ,receiver ,data);
	dispatchMessage(message);
...
void CCMessageManager::dispatchMessageWithType(MessageType type ,MessageParty sender ,MessageParty receiver)
{
	CCMessage* message=new CCMessage();
...
void CCMessageManager::dispatchMessage(CCMessage* message ,MessageParty  receiver)
{
	if (message->getType()!=GlobalMessageType) {
		//message for global
		...
		if (msgMap) {
			//parse for sender
			//如果sender不为空，则还要触发一次全局消息。
			MessageParty sender=message->getSender();
			if (sender) {
				//执行注册到sender的消息的处理方法
				CCDictionary* senderMap=(CCDictionary*)msgMap->objectForKey(sender->m_uID);
				//如果注册则执行
