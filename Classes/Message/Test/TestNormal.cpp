...
int main (int argc, const char * argv[])
{
	...
	DataItem datas[]={
		...
		{CarStop,nil,aMan,@selector(wait:),CarStop,aCar,aMan,nil},//no
	...
	for(int i=0;i<dataLength;i++){
		...
		//regiester
		NormalMessageManager * mm=[[NormalMessageManager alloc] init];
	...
	//dispatch
	Message *message=[[Message alloc] initWithType:CarStop sender:nil receiver:aMan data:nil];
	[mm dispatchMessage:message];
	
	gettimeofday(&end,NULL);
	NSLog(@"use:%ld,%d",end.tv_sec-start.tv_sec,end.tv_usec-start.tv_usec);
